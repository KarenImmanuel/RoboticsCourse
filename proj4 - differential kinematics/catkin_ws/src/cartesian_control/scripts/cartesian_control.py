...
def cartesian_control(joint_transforms, b_T_ee_current, b_T_ee_desired,
    ...
    #print('hello')'''
    #print([joint_transforms[i].shape for i in range(len(joint_transforms))])
    #-------------------- Fill in your code here ---------------------------
    
    
    ####Calculate the transform that goes from ee_current to ee_desired: 

    ee_cur_T_b=numpy.linalg.inv(b_T_ee_current)
    delta_x= numpy.dot(ee_cur_T_b,b_T_ee_desired )
    #delta= b_T_ee_desired - b_T_ee_current 
    #print(delta_x)
    delta_x_T=tf.transformations.translation_from_matrix(delta_x)
    #print(delta_x_T)
    angle,axis=rotation_from_matrix(delta_x)
    delta_x_R=angle*axis
    #print(numpy.transpose([delta_x_R]))
    
    ### X dot after scalar with random scalars for translation and rotation parts
    
    x_dot_T=4*numpy.transpose([delta_x_T])
    x_dot_R=2*numpy.transpose([delta_x_R])
    x_dot=numpy.vstack((x_dot_T,x_dot_R))
    #print(x_dot.shape)
    
    ## to calculate vee
      ###extract ee_R_b
        
    b_R_ee=tf.transformations.rotation_matrix(angle, axis)
    #print(b_R_ee)
    ee_R_b=numpy.linalg.inv(b_R_ee)
    #print(ee_R_b)
    
    ##calculating submatrices m1, m2, m3, m4
    
    m1=numpy.matrix([ee_R_b[0,:3],ee_R_b[1,:3],ee_R_b[2,:3]])
    add_zeros=numpy.zeros([3,3])
    m2=numpy.hstack((m1,add_zeros))
    m3=numpy.hstack((add_zeros,m1))
    m4=numpy.vstack((m2,m3))
    vee=numpy.dot(m4,x_dot)
    #print(vee)
    
    
    
    #norm??
    delta_x_norm= numpy.linalg.norm(delta_x)
    #print(delta_x)
    #print(delta_x_norm)
    ...
    # after this caclculate Vj from eeRj and jtee.  entire matrix [[eeRj , -eeRj.S(jTee)], [0, eeRj]]
    #jt=joint_transforms[::-1]
    #print(len(jt))
    #temp=[]
    Vj=[]
    
    ##calculate for each joint jTee by multiplying with inverse of previous joint transform. (required for Vj)
    
    for i in range(num_joints):
        #print('hiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii')
        #print(joint_transforms[i])
        
        #print('-----------------------------------------')
        #print(jt[6-i])
    #for i in range(len(joint_transforms),0,-1):
        #temp.append(functools.reduce(lambda x,y:numpy.dot(x,numpy.linalg.inv(y)),jt[:i+1]))
        ...
        skewed=S_matrix(j_t_ee)
        #ang,ax=rotation_from_matrix(temp[i])
        #j_R_ee=tf.transformations.rotation_matrix(ang, ax)
        jRee=numpy.matrix([j_T_ee[0,:3],j_T_ee[1,:3],j_T_ee[2,:3]])
        ee_R_j=numpy.linalg.inv(jRee)
        #print(ee_R_j.shape)
        my_mat2=numpy.dot(-1*(ee_R_j),skewed)
        
        #### store Vj for all joints
        
        Vj.append(numpy.vstack((numpy.hstack((ee_R_j,my_mat2)),numpy.hstack((add_zeros,ee_R_j)))))
        #print(Vj[i])
    
    ####calculate J by extracting columns for Vj
    
    J=numpy.hstack((Vj[0][:,5],Vj[1][:,5],Vj[2][:,5],Vj[3][:,5],Vj[4][:,5],Vj[5][:,5],Vj[6][:,5]))
    #print(J)
    J_plus=numpy.linalg.pinv(J,1e-2)
    #print(J_plus.shape)
    dq=numpy.dot(J_plus,vee)
    #print(dq)
    #J=numpy.hstack((Vj[
